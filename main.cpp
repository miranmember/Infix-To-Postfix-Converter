...
int doArithmetic(int x, int y, char symbol) {
...
void processExpression(Token inputToken, TokenReader * tr) {
    ...
    while (inputToken.equalsType(EOLN) == false) {
        if (inputToken.getOperator() == '(') {
        ...
        } else if (inputToken.equalsType(VALUE)) {
            ...
            while(!operators.isEmpty() && operators.top() != '(') {
                ...
                operators.pop();
                operands.push(doArithmetic(x, y, symbol));
            }
            if(!operators.isEmpty())
            	operators.pop();
        } else {
            while(!operators.isEmpty() && 
				  findPriority(operators.top()) >= findPriority(inputToken.getOperator())){
                int y = operands.top();
                operands.pop();
                int x = operands.top();
                operands.pop();
